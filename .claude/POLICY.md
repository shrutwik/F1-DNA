...
Project [`.claude/hooks.json`](hooks.json) adds size/output nudges. Cursor uses [`.cursor/hooks.json`](../.cursor/hooks.json) with `npx -y context-mode` for PreToolUse routing (see also [`.cursor/mcp.json`](../.cursor/mcp.json)).
