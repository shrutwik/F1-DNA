...
**Deep policy (optional read):** `[.claude/POLICY.md](.claude/POLICY.md)` — Superpowers workflow, response shape, Claude-only hooks.

**Subagents:** use root `[implementer-prompt.md](implementer-prompt.md)`, `[spec-reviewer-prompt.md](spec-reviewer-prompt.md)`, `[code-quality-reviewer-prompt.md](code-quality-reviewer-prompt.md)`. Orchestrator pastes full task text; subagent does not load plan files.
